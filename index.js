const inquirer = require("inquirer"); // 対話インターフェースモジュール
const chalk = require("chalk"); // 文字の色付け
const figlet = require("figlet"); // でっかいフォント
...
//アプリケーションの実行中に毎回新しいuserWalletを作成したくない場合は、
// 変数(userPublicKey & userSecretKey)に保存してください。
//秘密鍵からウォレットインスタンスを生成するには、次のようにします。
...
// getReturnAmount関数:  プレイヤーの推測が正しかった場合に、そのプレイヤーが得られる金額の合計を返します。
// totalAmtToBePaid関数: 各ゲームでプレイヤーが支払うべき金額の合計を返します。
// randomNumber関数:     パラメータとして渡された[min,max]の範囲にある数字を生成する関数です。
// getWalletBalance関数: 引数として渡されたウォレットの残高を返します。
// transferSOL 関数: 転送命令（あるウォレットから別のウォレットに SOL を転送するための命令）が含まれています。
// airDropSol関数:     Lamportsをウォレットにエアドロップするために使用されます（devnetでのみ動作します）。